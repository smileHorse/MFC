...
int CMainWindow::OnCreate( LPCREATESTRUCT lpCreateStruct )
{
	...
	// Set a timer to fire a 1-second intervals
...
void CMainWindow::OnContextMenu( CWnd* pWnd, CPoint pos )
{
	...
	if (rect.PtInRect(pos))
	{
		...
		int nID = (int)pMenu->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_RETURNCMD, pos.x, pos.y, this);
...
void CMainWindow::OnTimer( UINT nTimerID )
{
	...
	if ((nSecond == m_nPrevSecond) && (nMinute == m_nPrevMinute) && 
		(nHour == m_nPrevHour))
	{
		return;
	}
	...
	{
		DrawHand(&dc, 200, 4, (m_nPrevHour * 30) + (m_nPrevMinute / 2), clrColor);
	...
	if (nSecond != m_nPrevSecond)
	{
		...
		DrawHand(&dc, 200, 4, (nHour * 30) + (nMinute / 2), RGB(0, 0, 0));
...
void CMainWindow::OnPaint()
{
	...
	DrawClockFace(&dc);
	DrawHand(&dc, 200, 4, (m_nPrevHour * 30) + (m_nPrevMinute / 2), RGB(0, 0, 0));
...
void CMainWindow::DrawHand( CDC* pDC, int nLength, int nScale, int nDegress, COLORREF clrColor )
{
	CPoint point[4];
	double nRadians = (double)nDegress * 0.017453292;
...
void CMainWindow::DrawSecondHand( CDC* pDC, int nLength, int nScale, int nDegress, COLORREF clrColor )
{
	CPoint point[2];
	double nRadians = (double) nDegress * 0.017453292;
...
BOOL CMainWindow::RestoreWindowState()
{
	...
	if (((wp.flags = myApp.GetProfileInt(version, _T("flags"), -1)) != -1) && 
		((wp.showCmd = myApp.GetProfileInt(version, _T("showCmd"), -1)) != -1) && 
		((wp.rcNormalPosition.left = myApp.GetProfileInt(version, _T("x1"), -1)) != -1) && 
		((wp.rcNormalPosition.top = myApp.GetProfileInt(version, _T("y1"), -1)) != -1) && 
		((wp.rcNormalPosition.right = myApp.GetProfileInt(version, _T("x2"), -1)) != -1) && 
		((wp.rcNormalPosition.bottom = myApp.GetProfileInt(version, _T("y2"), -1)) != -1))
	{
